...
namespace Platform.Data.Doublets.Sequences
{
    ...
    /// <para></para>
    /// </summary>
    public partial class Sequences : ILinks<LinkIndex>
    {
        ...
        /// <para></para>
        /// </summary>
        public ILinks<LinkIndex> Links { get; }
        ...
        public Sequences(ILinks<LinkIndex> links, SequencesOptions<LinkIndex> options)
        {
            Links = links;
            Options = options;
            Options.ValidateOptions();
            
            // Handle synchronization based on options and link type
            if (Options.UseSynchronization && links is ISynchronizedLinks<LinkIndex> syncLinks)
            {
                _sync = syncLinks.SyncRoot;
                Options.InitOptions(syncLinks);
            }
            else
            {
                _sync = new NonSynchronization(); // No-op synchronization
                // For unsynchronized links, we need to create a wrapper for the InitOptions call
                if (links is ISynchronizedLinks<LinkIndex> syncLinksForInit)
                {
                    Options.InitOptions(syncLinksForInit);
                }
                else
                {
                    // Create a temporary synchronized wrapper just for initialization
                    var tempSyncLinks = new SynchronizedLinks<LinkIndex>(links);
                    Options.InitOptions(tempSyncLinks);
                }
            }
        ...
        /// </param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Sequences(ILinks<LinkIndex> links) : this(links, new SequencesOptions<LinkIndex>()) { }

        /// <summary>
        /// <para>
        /// Initializes a new <see cref="Sequences"/> instance with synchronized links.
        /// </para>
        /// <para></para>
        /// </summary>
        /// <param name="links">
        /// <para>A synchronized links.</para>
        /// <para></para>
        /// </param>
        /// <param name="options">
        /// <para>A options.</para>
        ...
        /// </param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Sequences(SynchronizedLinks<LinkIndex> links, SequencesOptions<LinkIndex> options) : this((ILinks<LinkIndex>)links, options) { }
        ...
        /// </param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Sequences(SynchronizedLinks<LinkIndex> links) : this((ILinks<LinkIndex>)links, new SequencesOptions<LinkIndex>()) { }
        ...
        public bool IsSequence(LinkIndex sequence)
        {
            return _sync.DoRead(() =>
            {
                ...
                return !GetUnsyncLinks().IsPartialPoint(sequence);
            });
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ILinks<LinkIndex> GetUnsyncLinks()
        {
            return Links is SynchronizedLinks<LinkIndex> syncLinks ? syncLinks.Unsync : Links;
        }
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public LinkIndex Create(IList<LinkIndex>? restriction, WriteHandler<LinkIndex>? handler)
        {
        ...
        {
            return Create(restriction, null);
        }
        ...
        private LinkIndex CreateCore(IList<LinkIndex>? restriction)
        {
            ...
            if (Options.UseSequenceMarker)
            {
                return GetUnsyncLinks().GetOrCreate(Options.SequenceMarkerLink, sequenceRoot);
            }
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public LinkIndex Each(IList<LinkIndex>? restriction, ReadHandler<LinkIndex>? handler)
        {
            return _sync.DoRead(() =>
            {
                if (restriction.IsNullOrEmpty())
                ...
                if (restriction.Count == 1)
                {
                    ...
                    if (link == any)
                    {
                        if (Options.UseSequenceMarker)
                        {
                            return GetUnsyncLinks().Each(new Link<LinkIndex>(any, Options.SequenceMarkerLink, any), handler);
                        }
                        else
                        {
                            return GetUnsyncLinks().Each(handler, new Link<LinkIndex>(any, any, any));
                        }
                    }
                    if (Options.UseSequenceMarker)
                    {
                        var sequenceLinkValues = GetUnsyncLinks().GetLink(link);
                ...
                {
                    return GetUnsyncLinks().Each(restriction, handler);
                }
                else
                {
        ...
        {
            return GetUnsyncLinks().Each(doublet =>
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private LinkIndex StepRight(Func<IList<LinkIndex>, LinkIndex> handler, LinkIndex left, LinkIndex right) => GetUnsyncLinks().Each(rightStep => TryStepRightUp(handler, right, rightStep[Constants.IndexPart]), new Link<LinkIndex>(Constants.Any, left, Constants.Any));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private LinkIndex TryStepRightUp(Func<IList<LinkIndex>, LinkIndex> handler, LinkIndex right, LinkIndex stepFrom)
        {
            var upStep = stepFrom;
            var firstSource = GetUnsyncLinks().GetTarget(upStep);
            while (firstSource != right && firstSource != upStep)
            {
                upStep = firstSource;
                firstSource = GetUnsyncLinks().GetSource(upStep);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private LinkIndex StepLeft(Func<IList<LinkIndex>, LinkIndex> handler, LinkIndex left, LinkIndex right) => GetUnsyncLinks().Each(leftStep => TryStepLeftUp(handler, left, leftStep[Constants.IndexPart]), new Link<LinkIndex>(Constants.Any, Constants.Any, right));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private LinkIndex TryStepLeftUp(Func<IList<LinkIndex>, LinkIndex> handler, LinkIndex left, LinkIndex stepFrom)
        {
            var upStep = stepFrom;
            var firstTarget = GetUnsyncLinks().GetSource(upStep);
            while (firstTarget != left && firstTarget != upStep)
            {
                upStep = firstTarget;
                firstTarget = GetUnsyncLinks().GetTarget(upStep);
            }
            if (firstTarget == left)
            {
                return handler(new LinkAddress<LinkIndex>(stepFrom));
        ...
        public LinkIndex Update(IList<LinkIndex>? restriction, IList<LinkIndex>? substitution, WriteHandler<LinkIndex>? handler)
        {
            ...
            {
                return Create(substitution, handler);
            }
            if (newSequence.IsNullOrEmpty())
            {
                Delete(restriction, handler);
                return Constants.Null;
            }
            return _sync.DoWrite(() =>
            {
        ...
        private void UpdateOneCore(LinkIndex sequence, LinkIndex newSequence, WriteHandler<LinkIndex>? handler)
        {
            if (Options.UseGarbageCollection)
            {
                ...
                if (Options.UseCascadeUpdate || CountUsages(sequence) == 0)
                {
                    if (sequenceLink != Constants.Null)
                    {
                        GetUnsyncLinks().MergeAndDelete(sequenceLink, newSequenceLink);
                    }
                    GetUnsyncLinks().MergeAndDelete(sequenceElements, newSequenceElements);
                }
            ...
            else
            {
                ...
                else
                {
                    if (Options.UseCascadeUpdate || CountUsages(sequence) == 0)
                    {
                        GetUnsyncLinks().MergeAndDelete(sequence, newSequence);
                    }
                }
            }
        }
        ...
        /// </param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public LinkIndex Delete(IList<LinkIndex>? restriction, WriteHandler<LinkIndex>? handler)
        {
            return _sync.DoWrite(() =>
            {
                var sequence = restriction.SkipFirst();
        ...
        /// </param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public LinkIndex Delete(IList<LinkIndex>? restriction)
        {
            return Delete(restriction, null);
        }
        ...
        private void DeleteOneCore(LinkIndex link)
        {
            if (Options.UseGarbageCollection)
            {
                ...
                if (Options.UseCascadeDelete || CountUsages(link) == 0)
                {
                    if (sequenceLink != Constants.Null)
                    {
                        GetUnsyncLinks().Delete(sequenceLink);
                    }
                    GetUnsyncLinks().Delete(link);
                }
                ClearGarbage(sequenceElementsContents.Source);
                ClearGarbage(sequenceElementsContents.Target);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private LinkIndex CompactCore(IList<LinkIndex> sequence) => UpdateCore(sequence, sequence, default!);
        ...
        public class Matcher : RightSequenceWalker<LinkIndex>
        {
            ...
            public Matcher(Sequences sequences, IList<LinkIndex> patternSequence, HashSet<LinkIndex> results, ReadHandler<LinkIndex> stopableHandler, HashSet<LinkIndex>? readAsElements = null)
                : base(sequences.GetUnsyncLinks(), new DefaultStack<LinkIndex>())
            {
        ...
        // Placeholder method for garbage collection functionality
        private void ClearGarbage(LinkIndex link)
        {
            // Implementation would depend on the specific garbage collection strategy
        }
    }

    // Simple non-synchronization implementation for when synchronization is disabled
    public class NonSynchronization : ISynchronization
    {
        public T DoRead<T>(Func<T> func) => func();
        public void DoRead(Action action) => action();
        public T DoWrite<T>(Func<T> func) => func();
